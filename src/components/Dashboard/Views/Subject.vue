              ...
              <form @submit.prevent="$emit('submitAction', item)">
                <div class="form-group row">
                  <label class="col-4 col-form-label"
                    >{{ $t("common.subject.input.title") }} :</label
                  >
                  <div class="col-8">
                    <input
...
export default {
    ...
    return {
      ...
      datatable: {
        columns: [
          ...
          {
            title: this.$t("common.subject.datatable.title"),
            field: "title",
  ...
  methods: {
    getAllSubjects() {
      ...
      this.loading = true;
      this.$store
        .dispatch("definition/getAllSubjects", { filter: filter })
        .then((response) => {
          this.loading = false;
      ...
      this.$modal.show("dialog", {
        ...
        buttons: [
          ...
          },
          {
            ...
            handler: () => {
              this.$store
                .dispatch("definition/deleteSubject", { data: item })
                .then(() => {
    ...
    saveSubject() {
      if (this.mode === "save") {
        this.$store
          .dispatch("definition/createSubject", { data: this.subject })
          .then((response) => {
            this.getAllSubjects();
        ...
        this.$store
          .dispatch("definition/updateSubject", { data: this.subject })
          .then((response) => {
            this.getAllSubjects();
            this.$modal.hide("subject-modal");
            this.notify(this.$t(response.message), "success");

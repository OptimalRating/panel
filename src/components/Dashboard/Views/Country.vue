    ...
    <div class="container-fluid">
      <div class="row">
        ...
        <div class="col-4">
          ...
          >
            <i class="fas fa-plus"></i> {{ $t("common.buttons.new") }}
          </button>
        </div>
      </div>
      <div class="row">
    ...
    return {
      ...
      datatable: {
        columns: [
          ...
          {
            title: this.$t("common.country.datatable.name"),
            field: "name",
            tdComp: TableCustomColumn,
          },
          {
            title: this.$t("common.country.datatable.code"),
            field: "code",
            tdComp: TableCustomColumn,
          },
          {
            title: this.$t("common.country.datatable.flag"),
            field: "flag",
            tdComp: ImageField,
          },
          {
            title: this.$t("common.country.datatable.order"),
            field: "sort_order",
            tdComp: TableCustomColumn,
          },
          {
            title: this.$t("common.country.datatable.status"),
            field: "status",
            tdComp: TableColumnStatus,
          },
          {
            tdComp: CountryTableActions,
            visible: "true",
            thStyle: { width: "10%" },
            tdStyle: { width: "10%" },
          },
...
>>>>>>> Stashed changes
  ...
  methods: {
    getAllCountries() {
      ...
      this.loading = true;
      this.$store
        .dispatch("definition/getAllCountries", { filter: filter })
        .then((response) => {
          this.loading = false;
      ...
      this.$modal.show("dialog", {
        ...
        buttons: [
          ...
          },
          {
            ...
            handler: () => {
              this.$store
                .dispatch("definition/deleteCountry", { data: item })
                .then(() => {
    ...
    saveCountry() {
      if (this.mode === "save") {
        this.$store
          .dispatch("definition/createCountry", { data: this.country })
          .then((response) => {
            this.getAllCountries();
        ...
        this.$store
          .dispatch("definition/updateCountry", { data: this.country })
          .then((response) => {

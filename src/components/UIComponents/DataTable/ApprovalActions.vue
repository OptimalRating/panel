<template>
  <div class="action-list">
    <button
      ...
      type="button"
      @click.prevent="actionClickHandler(row.status)"
    >
      {{ row.status == "active" ? "Reject" : "Approve" }} ({{ row.status }})
    </button>
  </div>
</template>
...
export default {
  ...
  methods: {
    actionClickHandler(key) {
      if (key == "active") this.row.status = "passive";
      else this.row.status = "active";
